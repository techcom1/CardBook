	...
	var cardbookDuplicate = {
		...
		loadDuplicate: function () {
			...
			if (cacheDir.exists()) {
				...
				cardbookSynchronization.getFileDataAsync(cacheDir.path, cardbookDuplicate.loadDuplicateAsync, params);
			} else {
		...
		writeDuplicate: function () {
			...
			if (cacheDir.exists()) {
				...
				myArray = cardbookRepository.arrayUnique(myArray);
				cardbookSynchronization.writeFileDataAsync(cacheDir.path, myArray.join("\r\n"), function () {
				wdw_cardbooklog.updateStatusProgressInformationWithDebug2("debug mode : Duplicate file written to : " + cacheDir.path);
				});
			}
		}
